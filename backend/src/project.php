...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST)) {
    try {
        ...
        // テーブル名を作成
        $tableName = "project_$userId";

        // テーブル名のホワイトリスト検証
        if (!preg_match('/^project_\d+$/', $tableName)) {
        ...
        if ($_POST['action'] === 'save') {
            ...
            if (!$name || !isset($end) || $completed === null || !isset($memo)) {
                throw new Exception('Invalid input data');
            }
            $stmt = $pdo->prepare("INSERT INTO $tableName (name, end, completed, memo) VALUES (?, ?, ?, ?)");
            $stmt->execute([$name, $end, $completed, $memo]);
            $projectId = $pdo->lastInsertId(); // 挿入されたデータのIDを取得
            // コミット
            $pdo->commit();
            ...
            if($type === 'end') {
            $end = sanitize($_POST['end']);
            if (!isset($end)) {throw new Exception('Invalid input data');}
            $stmt = $pdo->prepare("UPDATE $tableName SET end=? WHERE id=?");
            $stmt->execute([$end, $id]);
            }

            if($type === 'completed') {
            $completed = validateAndSanitizeInt($_POST['completed']);
            if ($completed === null) {throw new Exception('Invalid input data');}
            $stmt = $pdo->prepare("UPDATE $tableName SET completed=? WHERE id=?");
